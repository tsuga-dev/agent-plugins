...
4. Phase 7 — fan out one subagent per service in `/tmp/services-to-dossier.txt`. Batches of 8–12 in parallel. Prompt template is in `SUBAGENT_PROMPT.md`; copy verbatim, substitute `{svc}`, `{team}`, `{team_id}`, `{company}`, `{N}`, `{svc-prefix}` (drop that probe when the service has no metric namespace).
