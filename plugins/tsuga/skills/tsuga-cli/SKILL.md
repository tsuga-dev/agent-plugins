...
- Limit result count with the CLI's own flag, never `| head`: `--max-results` on `logs search`, `traces search` and `rum search`, `--limit` on the paginated resource lists. The raw-log cap under Safety still applies.
