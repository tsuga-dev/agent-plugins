# OTel Reference — Node.js

> **Last verified:** 2026-03-23 | SDK: `@opentelemetry/sdk-node` 0.213.x / `@opentelemetry/api` 1.9.x

Canonical reference for OpenTelemetry instrument types, naming rules, and telemetry best practices. Node.js-specific SDK API and environment variable details.
...
| **Synchronous Gauge** | Spot measurement recorded imperatively (call `.record()` per measurement) | No | Cache hit ratio per request, per-operation thread count |
...
## Node.js Instrument API

| Instrument | SDK Method | Notes |
|---|---|---|
| Counter | `meter.createCounter(name, options?)` | `options` has `unit`, `description`, `valueType` |
| Histogram | `meter.createHistogram(name, options?)` | Use for latency/duration |
| UpDownCounter | `meter.createUpDownCounter(name, options?)` | Supports negative delta |
| Synchronous Gauge | `meter.createGauge(name, options?)` | Call `.record(value, attrs)` imperatively |
| Observable Gauge | `meter.createObservableGauge(name, options?)` | Register callback via `observable.addCallback(cb)` |
| Observable Counter | `meter.createObservableCounter(name, options?)` | — |
| Observable UpDownCounter | `meter.createObservableUpDownCounter(name, options?)` | — |
...
> **SDK auto-reads:** `OTLPTraceExporter()` (zero-arg) reads `OTEL_EXPORTER_OTLP_ENDPOINT`. `NodeSDK` reads `OTEL_SERVICE_NAME` automatically. `OTEL_TRACES_EXPORTER` and `OTEL_METRICS_EXPORTER` default to `otlp` — the SDK auto-configures OTLP exporters when no exporter is passed in code.

| Variable | Purpose | Required? | Default (per spec) | SDK auto-reads? | Node.js notes |
|---|---|---|---|---|---|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP endpoint URL | **yes** | `http://localhost:4318` | yes | Zero-arg `OTLPTraceExporter()` reads this |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | Transport protocol | no | `http/protobuf` | yes | Set `grpc` only when targeting gRPC receiver on port 4317 |
| `OTEL_EXPORTER_OTLP_HEADERS` | Auth/custom headers | depends | (none) | yes | — |
| `OTEL_EXPORTER_OTLP_TIMEOUT` | Export timeout (ms) | no | `10000` | yes | — |
| `OTEL_EXPORTER_OTLP_COMPRESSION` | Compression | no | (unset) | yes | `gzip` recommended for production |
...
| `OTEL_TRACES_EXPORTER` | Traces exporter type | no | `otlp` | yes | `NodeSDK` auto-configures OTLP exporter when unset |
| `OTEL_METRICS_EXPORTER` | Metrics exporter type | no | `otlp` | yes | `NodeSDK` auto-configures OTLP exporter when unset |
| `OTEL_LOGS_EXPORTER` | Logs exporter type | no | `otlp` | yes | Logs SDK is Development status |
| `OTEL_SERVICE_NAME` | Service name | **yes** | (none) | yes | `NodeSDK` reads this automatically |
| `OTEL_RESOURCE_ATTRIBUTES` | Additional resource attrs | no | (none) | yes | — |
