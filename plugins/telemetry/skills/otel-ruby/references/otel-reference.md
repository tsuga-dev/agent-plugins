...
Canonical reference for OpenTelemetry instrument types, naming rules, and telemetry best practices. Ruby-specific SDK API and environment variable details.
...
| **Synchronous Gauge** | Current spot value recorded imperatively (call `.record()` on each measurement) | No | Per-request cache hit ratio, current thread count at point of measurement |
...
> **Status note:** Ruby Metrics and Logs SDKs are in Development status. Test SDK stability for your version before using in production.
...
|---|---|---|
| Counter | `meter.create_counter(name, unit:, description:)` | Returns `Counter` instrument |
...
| Synchronous Gauge | `meter.create_gauge(name, unit:, description:)` | Call `.record(value, attributes:)` imperatively; stable since SDK 1.5.x |
| Observable Gauge | `meter.create_observable_gauge(name, unit:, description:, &callback)` | Block receives `ObservableResult`; called at collection time |
| Observable Counter | `meter.create_observable_counter(name, unit:, description:, &callback)` | — |
| Observable UpDownCounter | `meter.create_observable_up_down_counter(name, unit:, description:, &callback)` | — |

**Recording values:**
```ruby
request_counter.add(1, attributes: { 'method' => 'GET', 'status' => '200' })
latency_histogram.record(42.5, attributes: { 'endpoint' => '/api' })
queue_depth.add(-1, attributes: { 'queue' => 'jobs' })
```

Get meter via: `meter = OpenTelemetry.meter_provider.meter('my-service', version: '1.0.0')`
...
> **SDK auto-reads:** The `opentelemetry-exporter-otlp` gem reads `OTEL_EXPORTER_OTLP_ENDPOINT` automatically. The Ruby SDK reads `OTEL_SERVICE_NAME` and `OTEL_RESOURCE_ATTRIBUTES` automatically — do NOT set `c.service_name` in code.

| Variable | Purpose | Required? | Default (per spec) | SDK auto-reads? | Ruby notes |
|---|---|---|---|---|---|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP endpoint URL | **yes** | `http://localhost:4318` | yes | Ruby gem defaults to HTTP/protobuf on port 4318 |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | Transport protocol | no | `http/protobuf` | yes | Ruby only supports `http/protobuf`; no gRPC exporter gem is published |
| `OTEL_EXPORTER_OTLP_HEADERS` | Auth/custom headers | depends | (none) | yes | — |
| `OTEL_EXPORTER_OTLP_TIMEOUT` | Export timeout (ms) | no | `10000` | yes | — |
| `OTEL_EXPORTER_OTLP_COMPRESSION` | Compression | no | `gzip` | yes | Ruby gem defaults to `gzip`; valid values: `gzip`, `none` |
| `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` | Traces-only endpoint | no | inherits `OTEL_EXPORTER_OTLP_ENDPOINT` | yes | — |
| `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT` | Metrics-only endpoint | no | inherits `OTEL_EXPORTER_OTLP_ENDPOINT` | yes | Metrics SDK is Development status |
| `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT` | Logs-only endpoint | no | inherits `OTEL_EXPORTER_OTLP_ENDPOINT` | yes | Logs SDK is Development status |
| `OTEL_TRACES_EXPORTER` | Traces exporter type | no | `otlp` | yes | — |
| `OTEL_METRICS_EXPORTER` | Metrics exporter type | no | (none) | partial | Metrics SDK is Development status |
| `OTEL_LOGS_EXPORTER` | Logs exporter type | no | `otlp` | yes | Logs SDK is Development status |
| `OTEL_SERVICE_NAME` | Service name | **yes** | (none) | yes | Auto-read; do NOT set `c.service_name` in code |
| `OTEL_RESOURCE_ATTRIBUTES` | Additional resource attrs | no | (none) | yes | Use for `deployment.environment.name`, etc. |
