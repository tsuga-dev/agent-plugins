...
| `telemetry.sdk.version` | Set by SDK | automatic | `1.x` |
...
| 2 | Language manifest | `composer.json` `"name"` field — use the part after `/` (e.g. `"vendor/my-service"` → `my-service`) |
...
Priority: CI/CD version variable (`GITHUB_SHA`, `CI_COMMIT_TAG`) → `composer.json` `"version"` field → `git describe --tags --always`.
...
## `ResourceInfoFactory::defaultResource()` — Reads Env Vars Automatically
...
// DEPRECATED: OpenTelemetry\SemConv\ResourceAttributes is deprecated.
// Use OpenTelemetry\SemConv\Attributes\ServiceAttributes (stable attributes)
// and OpenTelemetry\SemConv\Incubating\Attributes\ServiceIncubatingAttributes (incubating).
use OpenTelemetry\SemConv\ResourceAttributes;

// ResourceInfoFactory::defaultResource() reads OTEL_SERVICE_NAME and
// OTEL_RESOURCE_ATTRIBUTES automatically. This is the correct starting point.
// Do NOT hardcode service.name in code.
$resource = ResourceInfoFactory::defaultResource();
```

To add code-defined attributes (e.g., `service.version` from a build variable):

```php
// Preferred: use the new per-domain attribute interfaces
use OpenTelemetry\SemConv\Attributes\ServiceAttributes;
// For incubating attributes like SERVICE_NAMESPACE:
// use OpenTelemetry\SemConv\Incubating\Attributes\ServiceIncubatingAttributes;

$resource = ResourceInfoFactory::defaultResource()->merge(
    ResourceInfo::create(Attributes::create([
        ServiceAttributes::SERVICE_VERSION   => getenv('APP_VERSION') ?: '0.0.0',
        ServiceIncubatingAttributes::SERVICE_NAMESPACE => 'payments',
    ]))
);
```

> `merge()` merges the additional attributes into the default resource. The argument (updating resource) takes precedence: code-defined values override env var values if the same key appears in both. To let env vars win, swap the receiver and argument.
...
Then in the PHP bootstrap:

```php
$k8sResource = ResourceInfo::create(Attributes::create([
    'k8s.node.name'      => getenv('K8S_NODE_NAME') ?: '',
    'k8s.pod.name'       => getenv('K8S_POD_NAME') ?: '',
    'k8s.namespace.name' => getenv('K8S_NAMESPACE') ?: '',
]));

$resource = ResourceInfoFactory::defaultResource()->merge($k8sResource);
```
...
| `service.name` hardcoded in `ResourceInfo::create()` | Move to `OTEL_SERVICE_NAME`; use `ResourceInfoFactory::defaultResource()` |
...
Before modifying `ResourceInfo` or `ResourceInfoFactory` setup in source files:
