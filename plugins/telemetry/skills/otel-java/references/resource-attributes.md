...
| `telemetry.sdk.version` | Set by SDK | automatic | `1.60.1` |

> **Deprecated:** `deployment.environment` (without `.name`) was deprecated in OTel semconv v1.27.0.
...
| 2 | Language manifest | `pom.xml` `<artifactId>` or `settings.gradle` `rootProject.name` |
...
Priority: CI/CD version variable (`GITHUB_SHA`, `CI_COMMIT_TAG`) → `pom.xml` `<version>` (or Gradle `version` property) → `git describe --tags --always`.

### deployment.environment.name

Priority: existing OTel config → `SPRING_PROFILES_ACTIVE` (Spring Boot) or `APP_ENV` → Kubernetes namespace name.

---

## `Resource.getDefault()` — Reads Env Vars Automatically

```java
// Resource.getDefault() reads OTEL_SERVICE_NAME and OTEL_RESOURCE_ATTRIBUTES automatically.
// This is the correct starting point — do NOT hardcode service.name in code.
Resource resource = Resource.getDefault();
```

To add code-defined attributes (e.g., `service.version` from a properties file):
...
// Use namespace-specific classes: ServiceAttributes (stable) for service.name/version,
// DeploymentIncubatingAttributes (from opentelemetry-semconv-incubating) for deployment.environment.name.
Resource resource = Resource.getDefault().merge(
    Resource.builder()
        .put(ServiceAttributes.SERVICE_VERSION, BuildInfo.VERSION)
        .put("service.namespace", "payments")  // service.namespace not yet on a stable class
        .build()
);
```
...
```java
Resource k8sResource = Resource.builder()
    .put("k8s.node.name", System.getenv("K8S_NODE_NAME"))
    .put("k8s.pod.name", System.getenv("K8S_POD_NAME"))
    .put("k8s.namespace.name", System.getenv("K8S_NAMESPACE"))
    .build();

Resource resource = Resource.getDefault().merge(k8sResource);
```
...
| `service.name` hardcoded in code | Move to `OTEL_SERVICE_NAME`; use `Resource.getDefault()` |
