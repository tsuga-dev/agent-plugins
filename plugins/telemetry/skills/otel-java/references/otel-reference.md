...
Canonical reference for OpenTelemetry instrument types, naming rules, and telemetry best practices. Java-specific SDK API and environment variable details.
...
| **Synchronous Gauge** | Spot measurement recorded imperatively (call `.set()`) | No | Cache hit ratio per request, per-thread buffer fill level |
...
|---|---|---|
| Counter (long) | `meter.counterBuilder(name).build()` | Chain `.setUnit(...)`, `.setDescription(...)` before `.build()` |
| Counter (double) | `meter.counterBuilder(name).ofDoubles().build()` | Float variant |
| Histogram (double) | `meter.histogramBuilder(name).build()` | Use for latency/duration |
| Histogram (long) | `meter.histogramBuilder(name).ofLongs().build()` | Integer variant |
| UpDownCounter (long) | `meter.upDownCounterBuilder(name).build()` | Supports negative delta |
| Synchronous Gauge (double) | `meter.gaugeBuilder(name).build()` | Stable since SDK 1.38; call `.set(value, attributes)` to record. `gaugeBuilder()` already returns `DoubleGaugeBuilder` — no `.ofDoubles()` needed |
| Observable Gauge (double) | `meter.gaugeBuilder(name).buildWithCallback(m -> m.record(value))` | Callback invoked at collection time |
| Observable Counter (long) | `meter.counterBuilder(name).buildWithCallback(m -> m.record(value))` | — |
| Observable UpDownCounter (long) | `meter.upDownCounterBuilder(name).buildWithCallback(m -> m.record(value))` | — |

**Recording values:**
```java
counter.add(1, Attributes.of(AttributeKey.stringKey("method"), "GET"));
histogram.record(42.5, Attributes.of(AttributeKey.stringKey("endpoint"), "/api"));
```
...
> **SDK auto-reads:** `OtlpHttpSpanExporter.builder().build()` (no `.setEndpoint()`) reads `OTEL_EXPORTER_OTLP_ENDPOINT`. Java agent (v2.0+) reads ALL `OTEL_*` vars; default protocol is `http/protobuf`. `Resource.getDefault()` reads `OTEL_SERVICE_NAME` and `OTEL_RESOURCE_ATTRIBUTES`. `OtlpGrpcSpanExporter` requires the autoconfigure module for env-var support.

| Variable | Purpose | Required? | Default (per spec) | SDK auto-reads? | Java notes |
|---|---|---|---|---|---|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP endpoint URL | **yes** | `http://localhost:4318` | yes (Http builder); agent: yes | `OtlpGrpc*` without autoconfigure: **no** |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | Transport protocol | no | `http/protobuf` | yes (agent); programmatic: set by exporter class | — |
...
| `OTEL_TRACES_EXPORTER` | Traces exporter type | no | `otlp` | yes (agent); programmatic SDK: no | Must init exporter in code for programmatic setup |
| `OTEL_METRICS_EXPORTER` | Metrics exporter type | no | `otlp` | yes (agent); programmatic SDK: no | — |
| `OTEL_LOGS_EXPORTER` | Logs exporter type | no | `otlp` | yes (agent) | — |
| `OTEL_SERVICE_NAME` | Service name | **yes** | (none) | yes | `Resource.getDefault()` reads this |
| `OTEL_RESOURCE_ATTRIBUTES` | Additional resource attrs | no | (none) | yes | `Resource.getDefault()` merges these |
...
| Unit | UCUM | Use for |
|---|---|---|
| `s` | seconds | Latency, duration (semconv default for all duration metrics) |
| `ms` | milliseconds | Only when a non-semconv metric specifically measures milliseconds |
